...
import { getDiscoveryAvailabilitySettingsForAdmin } from "@/app/actions/discovery-scheduling-admin";
import { DiscoveryAvailabilityForm } from "@/components/admin/DiscoveryAvailabilityForm";
...
export const metadata = {
  title: "Discovery availability – Hargen Admin",
  robots: { index: false, follow: false },
};

export default async function DiscoveryAvailabilitySettingsPage() {
  await requireStaff();

  const [settings, readiness] = await Promise.all([
    getDiscoveryAvailabilitySettingsForAdmin(),
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Discovery availability</h1>
          <p className="text-muted-foreground">
            Configure when prospects can book discovery calls.
          </p>
        </div>
      </div>
