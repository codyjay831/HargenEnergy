...
import { DiscoverySchedulingLinkStatus } from "@/generated/prisma/client";
...
vi.mock("@/lib/discovery-scheduling/scheduling-readiness", () => ({
  getDiscoverySchedulingReadiness: () => mockGetReadiness(),
}));
...
vi.mock("@/lib/email", () => ({
  sendDiscoverySchedulingLinkEmail: (...args: unknown[]) => mockSendEmail(...args),
}));
...
vi.mock("@/lib/discovery-scheduling/tokens", () => ({
  ...
  buildDiscoverySchedulingUrl: (token: string) =>
    `https://app.example.com/schedule/discovery/${token}`,
}));

const { ensureDiscoverySchedulingLink } = await import(
  "@/lib/discovery-scheduling/ensure-scheduling-link"
);
...
describe("ensureDiscoverySchedulingLink", () => {
  ...
  it("returns error when discovery request is missing", async () => {
    ...
    expect(result).toEqual({ error: "Discovery request not found." });
  });

  it("creates a scheduling link and returns URL without sending email by default", async () => {
    const result = await ensureDiscoverySchedulingLink({
      supportRequestId: "req-1",
      audit: { action: "discovery.scheduling_link.auto_created" },
    });

    expect(result).toEqual({
      schedulingUrl: "https://app.example.com/schedule/discovery/test-raw-token",
    });
  ...
  it("reuses active link when errorIfActiveExists is false", async () => {
    ...
    expect(result).toEqual({
      schedulingUrl: "https://app.example.com/schedule/discovery/existing-token",
    });
    expect(mockCreateLink).not.toHaveBeenCalled();
  });
