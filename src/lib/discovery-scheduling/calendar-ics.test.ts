...
describe("discovery calendar ICS", () => {
  const startUtc = new Date("2026-05-27T20:30:00.000Z");
  const endUtc = new Date("2026-05-27T21:00:00.000Z");
  const uid = discoveryEventUid("appt_123");

  it("builds a publish ICS with stable UID and UTC timestamps", () => {
    const ics = buildDiscoveryIcs({
      uid,
      summary: buildDiscoveryEventSummary("Struxient"),
      description: "Discovery with Hargen Energy for Struxient.",
      startUtc,
      endUtc,
    ...
    expect(ics).toContain("SUMMARY:Hargen discovery — Struxient");
