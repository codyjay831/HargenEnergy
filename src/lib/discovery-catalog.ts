...
export type DiscoveryCatalogTask = {
  id: string;
  name: string;
  description: string | null;
};

export type DiscoveryCatalogCategory = {
  id: string;
  name: string;
  tasks: DiscoveryCatalogTask[];
};

export async function getPublicDiscoveryCatalog(): Promise<DiscoveryCatalogCategory[]> {
  const categories = await prisma.serviceCategory.findMany({
    ...
    select: {
      ...
      tasks: {
        ...
        orderBy: [{ discoveryOrder: "asc" }, { basePriority: "asc" }, { name: "asc" }],
      },
    },
    orderBy: { name: "asc" },
  });
...
export async function countPublicDiscoveryTasks(): Promise<number> {
...
export async function validateRequestedDiscoveryTaskIds(
