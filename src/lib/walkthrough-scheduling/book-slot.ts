...
import { createWalkthroughCalendarEvent } from "@/lib/google-calendar/events";
...
} from "@/lib/walkthrough-scheduling/slot-generator";

export type BookWalkthroughSlotInput = {
...
export function buildReminderRows(
  ...
  const rows: Array<{
    appointmentId: string;
    type: WalkthroughReminderType;
    channel: WalkthroughReminderChannel;
    scheduledFor: Date;
    status: WalkthroughReminderStatus;
  }> = [];

  rows.push({
    appointmentId,
    type: WalkthroughReminderType.CONFIRMATION,
  ...
  if (twentyFourHour > now) {
    rows.push({
      appointmentId,
      type: WalkthroughReminderType.TWENTY_FOUR_HOUR,
      channel: WalkthroughReminderChannel.EMAIL,
  ...
  if (oneHour > now) {
    rows.push({
      appointmentId,
      type: WalkthroughReminderType.ONE_HOUR,
      channel: WalkthroughReminderChannel.EMAIL,
...
export async function buildSlotGeneratorContext(
  ...
  const existingAppointments = await prisma.walkthroughAppointment.findMany({
    where: {
      status: {
...
export async function bookWalkthroughSlotAtomic(
  input: BookWalkthroughSlotInput,
  ...
  try {
    const appointmentId = await prisma.$transaction(async (tx) => {
      ...
      const created = await tx.walkthroughAppointment.create({
        data: {
